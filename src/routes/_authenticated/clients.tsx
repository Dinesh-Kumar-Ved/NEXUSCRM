...
function ClientsPage() {
  ...
  const handleClientSaved = async (savedClient: ClientRecord) => {
    ...
    if (import.meta.env.DEV) {
      console.group("ADD CLIENT LIST DEBUG");
      console.log("query key:", ["clients", workspaceId]);
      console.log("refreshed query result:", refreshed);
      console.log("saved row appears in query:", appearsInQuery);
      console.groupEnd();
    }
    if (!appearsInQuery) {
      ...
      {workspaceLoading || clientsQuery.isLoading ? (
        ...
        <div className="overflow-hidden rounded-xl border bg-card">
          ...
          {filteredClients.map((client) => (
            <div
              key={client.id}
              className="grid gap-3 border-b px-4 py-4 last:border-b-0 lg:grid-cols-[1.4fr_1fr_1.2fr_1fr_1fr_1fr_auto] lg:items-center lg:gap-4"
            >
              <Link
                to="/clients/$clientId"
